...
public class GoPluginImplTest {
    ...
    public void shouldGetCorrectDockerCmd() {
        ...
        assertThat(plugin.getDockerParam("pipelines/demo", "node6", "", "mongo-server:182.92.6.60", true),
                is("docker run --rm -v /run/go-agent/pipelines/demo:/workspace -w /workspace  --add-host mongo-server:182.92.6.60 node6 sh -c "));

        assertThat(plugin.getDockerParam("pipelines/demo", "node6", "VAR1=VAL1;VAR2=VAL2", "mongo-server:182.92.6.60;mysql-server:182.92.6.60", true),
                is("docker run --rm -v /run/go-agent/pipelines/demo:/workspace -w /workspace  -e VAR1=VAL1 -e VAR2=VAL2 --add-host mongo-server:182.92.6.60 --add-host mysql-server:182.92.6.60 node6 sh -c "));
    }
}